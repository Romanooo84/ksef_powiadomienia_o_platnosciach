...
log("Otrzymano challenge KSeF");

const key = await getPublicKey();
log("Pobrano klucz publiczny KSeF.")

const token= cryptoToken(challengeResponse,key);
log("Wygenerowano zaszyfrowany token KSeF: ");
...
while (true) {
  ...
  log("Status autoryzacji KSeF: 100 (w trakcie). Czekam...");
...
if (status === 200) {
  ...
  log("Uzyskano token dostępu KSeF.");
